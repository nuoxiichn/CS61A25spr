...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    res = 3 * abs(player_score % 10 - opponent_score // 10 % 10)
    if res > 0:
        return res
    else:
        return 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for player_score in range(goal + 1):
        for opponent_score in range(goal):
            if strategy(player_score, opponent_score) != strategy(player_score, opponent_score + 1):
                return False
    return True
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if sus_update(0, score, opponent_score) > GOAL:
        ...
        if boar_brawl(score, opponent_score) >= make_averaged(roll_dice, 1000)(6, six_sided):
            return 0
        else:
            return 6
    # END PROBLEM 12
